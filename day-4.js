const dataFromAOC = `eyr:2029 iyr:2013
...
const littleRaw = `ecl:gry pid:860033327 eyr:2020 hcl:#fffffd
...
// Replace a new line in the string with a space, with a proximity of 8 times
// this becouse we need several replaces of '\n' to 'spaces' in our strings
function replNewLineWithSpace (rawInput) {
  for (let prox = 0; prox <= 8; prox++) {
    rawInput = rawInput.replace('\n', ' ');
  }
  return rawInput;
}

// Convert every string in the raw data into an array of n elements each. It
// needs 8 (or 7 && !cid) for resulting valid. Then, return it as formatted
// array
function replEveryString (rawInput) {
  const formattedArr = [];
  rawInput.map(el => {
    formattedArr.push(replNewLineWithSpace(el));
  })
  return formattedArr;
}

// Get the values from every single row and puts it into a multidimensional
// array
function arrayToMultidim(array) {
  const multidimArr = [];
  for (row of array) {
    multidimArr.push(row.split(' '))
  }
  return multidimArr;
}

// The sum of all the function that transform raw data.
function turnRawDataInWorkable (dataFromAOC) {
  return arrayToMultidim(replEveryString(dataFromAOC));
}

function checkPassport (dataSet) {
  for (n in dataSet) {
  }
  console.log(dataSet);
}

const dataSet = turnRawDataInWorkable(dataFromAOC);


checkPassport(dataSet)
